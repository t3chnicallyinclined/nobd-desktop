...
/// DInput fightstick layout: attacks are buttons 0-7.
const DI_ATTACK_MASK: u16 = 0x00FF;
...
static DI_SW: LazyLock<[Mutex<SyncWindow>; NUM_PLAYERS]> =
    LazyLock::new(|| [Mutex::new(SyncWindow::new()), Mutex::new(SyncWindow::new())]);
    ...
    if hr == 0 && matches!(cb_data, 80 | 272) && !lpv_data.is_null() {
        unsafe {
            ...
            let now_us = EPOCH
                ...
                .elapsed()
                .as_micros() as u64;
            let window_us = crate::config::window_ms_u32(slot).clamp(1, 16) * 1000;
            if let Ok(mut sw) = DI_SW[slot].lock() {
                let filtered = sw.process(
                    raw,
                    DI_ATTACK_MASK,
                    DI_ATTACK_MASK,
                    now_us,
